...
const App = {
  ...
  renderEstoque() {
    ...
    return `<div class="screen-header"><div><h2>Estoque</h2><div class="list-item-sub">Peças disponíveis para venda</div></div><button class="btn btn-primary btn-sm" id="btn-novo-estoque">+ Cadastrar</button></div>
      <div class="stat-grid"><div class="stat"><div class="stat-value">${totalQtd}</div><div class="stat-label">Peças</div></div><div class="stat"><div class="stat-value">${this.money(totalCusto)}</div><div class="stat-label">Custo estimado</div></div><div class="stat"><div class="stat-value">${this.money(totalVenda)}</div><div class="stat-label">Valor de venda</div></div><div class="stat"><div class="stat-value">${this.money(totalVenda-totalCusto)}</div><div class="stat-label">Lucro potencial</div></div></div>
      ${estoque.length?estoque.map(x=>{const q=Number(x.quantidade)||0,c=this.stockUnitCost(x),pv=Number(x.precoVenda||this.config.produtos[x.produto]?.venda)||0,h=this.getHistoricoImposto(x.produto);return `<div class="list-item"><div class="list-item-info"><div class="list-item-title">${this.esc(x.time||x.produto)} — ${this.esc(x.tamanho||'')}</div><div class="list-item-sub">${this.esc(x.produto)} • ${q} un. • Custo ${this.money(c)}/un. • Imposto ${x.impostoPorPeca!=null?this.money(x.impostoPorPeca):this.money(h.taxPerPiece)} ${x.impostoPorPeca==null?'(estimado)':''}</div></div><div><span class="badge ${q>0?'badge-success':'badge-warning'}">${q} disponível</span><button class="btn btn-primary btn-sm mt-8" data-vender-estoque="${x.id}" ${q<=0?'disabled':''}>Vender</button><button class="btn btn-secondary btn-sm mt-8" data-editar-estoque="${x.id}">Editar</button></div></div>`}).join(''):`<div class="empty"><div class="icon">👕</div><p>Estoque vazio</p></div>`}`;
  },
  ...
  renderPrePedidos() {
    ...
    return `<div class="screen-header"><div><h2>Pré-pedidos</h2><div class="list-item-sub">Acumule peças de vários clientes no mesmo lote</div></div><button class="btn btn-primary btn-sm" id="btn-novo-prepedido">+ Novo</button></div>
      ...
      ${pps.length?pps.map(pp=>{
        const c=Storage.getCliente(pp.clienteId); const q=Calc.totalPecas(pp.itens||[]); const local=Calc.isMauaRegiao(c?.cidade||'');
        const pode=local?localPronto:true; const texto=local?`${totalLocal} de ${meta}${localPronto?' • Pedido liberado':''}`:`${q} peça(s) • Pedido liberado`;
        return `<div class="list-item"><div class="list-item-info"><div class="list-item-title">${this.esc(c?.nome||'Cliente')}</div><div class="list-item-sub">${q} peça(s) • ${this.esc((pp.itens||[]).map(i=>i.time||i.produto).join(', '))} • ${this.esc(c?.cidade||'')}</div></div><div><span class="badge ${pode?'badge-success':'badge-warning'}">${texto}</span>${pode?`<button class="btn btn-primary btn-sm mt-8" data-formar-pre="${pp.id}">📦 Efetuar pedido</button>`:''}</div></div>`;
  ...
  openPrePedidoForm() {
    ...
    if(!clientes.length)return this.toast('Cadastre um cliente primeiro!');
    const produtos=Object.keys(this.config.produtos), tamanhos=['P','M','G','GG','2XL','3XL','4XL'];
    this.openModal('Novo Pré-pedido',`
      <div class="form-group"><label>Cliente *</label><select id="pp-cliente"><option value="">Selecione...</option>${clientes.map(c=>`<option value="${c.id}">${this.esc(c.nome)} — ${this.esc(c.cidade)}</option>`).join('')}</select></div>
      <div class="section-title">Peças do cliente</div><div id="pp-itens"></div>
    ...
    const add=()=>{
      ...
      <div class="form-group"><label>Foto da camisa</label><input class="pp-foto" type="file" accept="image/*" capture="environment"></div>
    ...
    document.getElementById('btn-salvar-pp').onclick=async()=>{
      const clienteId=document.getElementById('pp-cliente').value;if(!clienteId)return this.toast('Selecione o cliente');
      const itens=[];
      for(const card of cont.querySelectorAll('.item-card')){
        itens.push({produto:card.querySelector('.pp-prod').value,tamanho:card.querySelector('.pp-tam').value,quantidade:Number(card.querySelector('.pp-qtd').value)||1,time:card.querySelector('.pp-time').value.trim(),valorVenda:Number(card.querySelector('.pp-venda').value)||0,desconto:Number(card.querySelector('.pp-desc').value)||0,dataVenda:card.querySelector('.pp-data-venda').value || new Date().toISOString().slice(0,10),personalizacao:card.querySelector('.pp-pers').checked,patch:card.querySelector('.pp-patch').checked,patrocinio:card.querySelector('.pp-patro').checked,feminina:card.querySelector('.pp-feminina').checked,fotoRef:await this.readFile(card.querySelector('.pp-foto').files[0])});
      }
      if(!itens.length)return this.toast('Adicione pelo menos uma peça');
      Storage.savePrePedido({clienteId,itens,observacoes:document.getElementById('pp-obs').value.trim(),status:'aberto'});
      this.closeModal();this.toast('Pré-pedido salvo!');this.render();
    };
  },

  openPrePedidoDetail(id) {
    const pp=Storage.getPrePedido(id);if(!pp)return;const c=Storage.getCliente(pp.clienteId),q=Calc.totalPecas(pp.itens||[]),meta=Number(this.config.pedido?.quantidadeMinima||4),local=Calc.isMauaRegiao(c?.cidade||''),locais=Storage.getPrePedidos().filter(x=>x.status==='aberto'&&Calc.isMauaRegiao(Storage.getCliente(x.clienteId)?.cidade||'')),totalLocal=locais.reduce((s,x)=>s+Calc.totalPecas(x.itens||[]),0),pode=local?totalLocal>=meta:true;
    this.openModal(`Pré-pedido — ${this.esc(c?.nome||'')}`,`
      <div class="card"><b>${this.esc(c?.nome||'')}</b><div class="list-item-sub">@${this.esc(c?.instagram||'')} • ${this.esc(c?.cidade||'')} • ${this.esc(c?.whatsapp||'')}</div></div>
  ...
  openPedidoDetail(id) {
    ...
    this.openModal(`Pedido #${p.numero}`,`
      ...
      <div class="photo-upload" id="ped-photo-area"><p>📷 Anexar foto</p><input id="ped-photo" type="file" accept="image/*" capture="environment" style="display:none"></div>
