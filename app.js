...
const App = {
  ...
  openCompartilharEstoque() {
    ...
    const tab=Storage.getTabelaMedidas();
    this.openModal('Compartilhar estoque',`
      ...
      <div class="form-group"><label style="display:flex;align-items:center;gap:8px"><input type="checkbox" id="share-medidas" ${tab.imagem||tab.texto?'checked':''}> Incluir tabela de medidas</label></div>
    ...
    const build=()=>{
      ...
      if(document.getElementById('share-medidas').checked){
        txt+='\n📏 *Guia de medidas*\n';
  ...
  openTabelaMedidas() {
    const tab=Storage.getTabelaMedidas();
    this.openModal('Tabela de medidas',`
      <p class="list-item-sub mb-12">Fica salva no app e pode ir no compartilhamento.</p>
      <div class="form-group"><label>Texto do guia</label><textarea id="tm-texto" rows="5" placeholder="Ex.: P 48-50cm peito...">${this.esc(tab.texto||'')}</textarea></div>
      <div class="form-group"><label>Foto da tabela</label><input id="tm-foto" type="file" accept="image/*">${tab.imagem?`<img src="${tab.imagem}" class="photo-preview">`:''}</div>
      <button class="btn btn-primary" id="btn-salvar-tm">Salvar</button>
      ${tab.imagem||tab.texto?'<button class="btn btn-danger mt-12" id="btn-limpar-tm">Limpar</button>':''}`);
    document.getElementById('btn-salvar-tm').onclick=async()=>{
      const f=document.getElementById('tm-foto').files[0];
      Storage.saveTabelaMedidas({texto:document.getElementById('tm-texto').value.trim(),imagem:f?await this.readFile(f):(tab.imagem||null)});
      this.closeModal();this.toast('Tabela salva!');
    };
    const bl=document.getElementById('btn-limpar-tm');
    if(bl)bl.onclick=()=>{Storage.saveTabelaMedidas({texto:'',imagem:null});this.closeModal();this.toast('Limpa');};
  },

  openDevedores() {
    const lista=Storage.getDevedores().sort((a,b)=>(a.status==='aberto'?0:1)-(b.status==='aberto'?0:1));
    this.openModal('Devedores',`
      <button class="btn btn-primary btn-sm mb-12" id="btn-novo-dev">+ Registrar dívida</button>
      ${lista.length?lista.map(d=>`
        <div class="list-item ${d.status==='aberto'?'pendencia':''}">
          <div class="list-item-info">
            ...
            <div class="list-item-sub">${this.esc(d.descricao||'')} • Restante ${this.money(d.valorRestante)}${d.prazo?' • Prazo '+new Date(d.prazo+'T12:00:00').toLocaleDateString('pt-BR'):''}</div>
          </div>
    ...
    document.getElementById('btn-novo-dev').onclick=()=>this.openNovoDevedor();
    document.querySelectorAll('[data-quitar-dev]').forEach(b=>b.onclick=()=>{
      ...
      Storage.saveDevedor(d);this.toast('Quitado!');this.openDevedores();
    });
    document.querySelectorAll('[data-excluir-dev]').forEach(b=>b.onclick=()=>{
      if(confirm('Excluir?')){Storage.deleteDevedor(b.dataset.excluirDev);this.openDevedores();}
    });
  },
  ...
  openNovoDevedor() {
    ...
    document.getElementById('btn-salvar-dev').onclick=()=>{
      ...
      this.toast('Registrado!');this.openDevedores();
    };
  },
